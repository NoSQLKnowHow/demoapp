...
│   JSON Collection Table (MAINTENANCE_CREW)        │
...
- **MAINTENANCE_CREW** — A JSON collection table storing crew records as JSON documents. Demonstrates Oracle's native JSON document storage alongside relational tables.
- **MAINTENANCE_LOGS** — Free-text maintenance and incident reports tied to infrastructure assets and maintenance crews. Narrative content is chunked and embedded for vector search.
...
| JSON (Native) | MAINTENANCE_CREW                               | JSON Collection Table             | Native JSON document storage in Oracle                        |
...
- Browse MAINTENANCE_CREW records as native JSON documents from the JSON collection table.
...
- Contrasts the Duality View approach with the JSON collection table (MAINTENANCE_CREW): two valid patterns for different use cases, both native to Oracle.
...
**Demo Mode:**
- **New Maintenance Log form:** Select an asset, select a maintenance crew (from the JSON collection), enter severity, and write the narrative text. On submit, the log is inserted, chunked, embedded, and stored in a single operation. A confirmation displays the new log and confirms it is now searchable via vector search.
...
│   ├── crews/                        GET — List maintenance crews (JSON collection)
│   └── crews/{id}                    GET — Single crew document
│
├── graph/
...
**Example MAINTENANCE_CREW document:**

```json
{
  "crewId": "CREW-017",
  "name": "Delta Maintenance Unit",
  "lead": "Reyes, M.",
  "specialty": "electrical infrastructure",
  "members": 6,
  "certifications": ["high-voltage", "confined-space", "hazmat-level-2"]
}
```
...
CREATE TABLE maintenance_logs (
    ...
    crew_id      VARCHAR2(50),   -- references crewId in MAINTENANCE_CREW JSON collection
...
2. Creates all canonical tables (DISTRICTS, INFRASTRUCTURE_ASSETS, MAINTENANCE_CREW, MAINTENANCE_LOGS, INSPECTION_REPORTS, INSPECTION_FINDINGS, ASSET_CONNECTIONS, DOCUMENT_CHUNKS).
...
Run `prism-seed.sql` or `python prism-seed.py` to populate all tables with CityPulse sample data. This includes inserting JSON documents into the MAINTENANCE_CREW collection table and JSON specifications into INFRASTRUCTURE_ASSETS.
...
| MAINTENANCE_CREW        | 8-12                  | JSON documents with varied specialties and certifications |
...
1. **See the Unified Model Theory in action** by observing the same CityPulse data served as relational rows, JSON documents (both Duality Views and native JSON collections), graph relationships, and vector search results, all from one database.
