...
> **Running Oracle Autonomous Database (ADB) in OCI instead?** This guide does not apply to you. ADB has no filesystem access to the database server, so you must stage the model through OCI Object Storage. See [../adb/load-prism-database-adb.md](../adb/load-prism-database-adb.md) for that approach.
